...
# Navigate to the project root if the script is in a subdirectory
project_root = os.path.join(script_dir, '..')  # Adjust the number of '..' based on the depth of the script
...
def main():
    # Print the current working directory to understand where the relative paths start
    print("Current Working Directory:", os.getcwd())

    # Now relative paths will start from the project root
    data_input_path = 'data/input/input00.txt'
    data_output_path = 'data/output/output00.txt'

    try:
        # Parsing the data
        data_df, missing_indices = parse_data(data_input_path)
    except FileNotFoundError:
        print(f"Could not find the file at {data_input_path}")
        return
